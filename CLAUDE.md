...
- **Kiosk UI** — unauthenticated route group `src/app/(kiosk)/kiosk/*` (home / setup / check-in / check-out), feature slice [features/kiosk](apps/web/src/features/kiosk) using a `kioskFetch` (device-token header, not the JWT cookie). Token paired once via `/kiosk/setup`. Admin **Devices** page + Host pre-register surface the entry code.
...
- **Premium kiosk flow** — check-in is: type → (pre-reg code | walk-in info form: name/phone/email + host search + purpose) → **live photo capture with retake** ([photo-capture.tsx](apps/web/src/features/kiosk/components/photo-capture.tsx), `getUserMedia` + face guide) → **ground rules/policy** then an **Agree & Sign modal** with a realtime canvas **signature pad** ([signature-pad.tsx](apps/web/src/features/kiosk/components/signature-pad.tsx)) → submit. Photo + signature are base64 → Cloudinary (`uploadHeadshot`/`uploadSignature`), with a **data-URL fallback** stored inline when Cloudinary isn't configured (dev). This supersedes PRD2 §6.4's "tap-to-agree, no signature" decision (the product now wants a drawn signature).
