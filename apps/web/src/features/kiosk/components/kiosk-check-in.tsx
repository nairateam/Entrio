...
import type { CheckInInput, CheckInResult, KioskHost, KioskVisit } from '../types';
...
export function KioskCheckIn() {
  ...
  const [prereg, setPrereg] = useState<KioskVisit | null>(null);

  const [info, setInfo] = useState({ fullName: '', phone: '', email: '', purpose: '' });
  const [host, setHost] = useState<KioskHost | null>(null);
  ...
  const lookupCode = useMutation({
    mutationFn: () => kioskApi.lookupByCode(code),
    onSuccess: (v) => {
      setPrereg(v);
      setStep('photo');
    },
  ...
  const stepper =
    stepIndex[step] !== undefined ? <KioskStepper steps={STEPS} current={stepIndex[step]!} /> : undefined;

  // --- Step: visitor type ---------------------------------------------------
  if (step === 'type') {
    return (
      <KioskShell key="type" title="Check in" subtitle="Are you pre-registered, or a walk-in?" onBack={() => history.back()}>
        <div className="grid gap-3.5 sm:grid-cols-2">
          <KioskButton size="lg" className="h-28 flex-col gap-2 text-lg" onClick={() => setStep('code')}>
            <LogIn className="h-6 w-6" />
            I&apos;m pre-registered
          </KioskButton>
          <KioskButton kioskVariant="soft" size="lg" className="h-28 flex-col gap-2 text-lg" onClick={() => setStep('info')}>
    ...
    return (
      ...
      >
        ...
        >
          ...
          <KioskButton kioskVariant="ghost" size="lg" className="w-full" onClick={() => setStep('type')} type="button">
            Back to options
          </KioskButton>
        </form>
  ...
  if (step === 'info') {
    ...
    return (
      ...
      >
        ...
        <KioskButton size="lg" className="mt-5 h-12 w-full" disabled={!valid} onClick={() => setStep('photo')}>
  ...
  if (step === 'photo') {
    return (
      ...
      >
        ...
        <KioskButton size="lg" className="mt-6 h-12 w-full" disabled={!photo} onClick={() => setStep('rules')}>
  ...
  if (step === 'rules') {
    return (
      <KioskShell key="rules" stepper={stepper} align="left" onBack={() => setStep('photo')}>
        ...
        <Modal open={signOpen} onClose={() => (submit.isPending ? null : setSignOpen(false))} size="lg">
          ...
          <ModalFooter>
            <KioskButton kioskVariant="ghost" onClick={() => setSignOpen(false)} disabled={submit.isPending}>
              Cancel
            </KioskButton>
            <KioskButton
